...
import { IPreviousModalProps } from "../../../../interface/IComponents/IModals/IPreviousModal";

const PreviousModal = ({ game_id, result, winner_balance, winner_list }: IPreviousModalProps) => {
    ...
    <tbody>
      <tr>
        ...
        <td className="text-right">
          <b>{Math.max(winner_balance, 0).toFixed(6)}</b>
        </td>
        <td className="text-right">
          {winner_balance > 0 ? (
            <StyledDetailsBtn
      ...
      {expanded && [
        <StyledTrSubheader key={`sub${game_id}`}>
          <th />
          <th className="text-left">TxID</th>
          <th className="text-right">Prize (BSV)</th>
          <th />
        </StyledTrSubheader>,
        ...winner_list.map((d: any) => (
          <tr key={d.tx_id}>
            <td />
            <td className="text-left">
              <a href={d.tx_id_url} target="_blank" rel="noopener noreferrer">
                <b>
            ...
            <td className="text-right">
              {d.payout_tx_id_url ? (
                <a href={d.payout_tx_id_url} target="_blank" rel="noopener noreferrer">
